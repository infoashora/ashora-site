...
export default function ProductCard({ product, variant = "default" }: Props) {
  ...
  // Heuristic: treat products with "Intention" in the badge as intention candles
  ...
  return (
    <div className="group relative h-full">
      {/* ✧ Always-on soft gold glow behind the card */}
      <div
        className="pointer-events-none absolute -inset-3 rounded-3xl blur-2xl"
        style={{
          background:
            "radial-gradient(circle at 20% 0%, rgba(209,169,84,0.45), transparent 60%)",
        }}
        aria-hidden="true"
      />
      ...
      {/* Actual card */}
      <div
        className={`relative z-10 flex h-full flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm ${outerState}`}
      >
        {/* SOLD OUT STATE */}
        {soldOut ? (
          <div
