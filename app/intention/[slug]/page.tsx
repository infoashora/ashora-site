...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: `${title} · ASHORA`, description };
}
...
export default async function IntentionPage({ params }: PageProps) {
  ...
  // Live products filtered by intention (up to 6)
    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      <section className="mt-8 grid gap-6 md:grid-cols-2">
        <div className="rounded-2xl border border-zinc-200 bg-white p-6">
          ...
          <div
            className="prose prose-sm mt-3 max-w-none text-zinc-700 prose-p:leading-6"
            dangerouslySetInnerHTML={{ __html: data.journeyHtml }}
          />
